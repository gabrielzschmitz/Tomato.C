...
void readLog(appData *app) {
  ...
  if (strftime(app->date, 50, "%d/%m/%Y", ptr_time) == 0) {
    ...
    while (fgets(line, sizeof line, log))
      if (strstr(line, app->date)) app->newDay = 0;
  }
  ...
  if (strstr(lastline, "WT") != NULL || strstr(lastline, "SP") != NULL ||
      strstr(lastline, "LP") != NULL)
    app->needResume = 1;

  /* Get unfinished variables */
  sscanf(lastline, "%d/%d WT %d D %d %*d %*d", &app->unfinishedPomodoroCounter,
         &app->unfinishedPomodoros, &app->unfinishedTimer,
         &app->unfinishedFullTimer);
  sscanf(lastline, "%d/%d SP %d D %*d %d %*d", &app->unfinishedPomodoroCounter,
         &app->unfinishedPomodoros, &app->unfinishedTimer,
         &app->unfinishedFullTimer);
  sscanf(lastline, "%d/%d LP %d D %*d %*d %d", &app->unfinishedPomodoroCounter,
         &app->unfinishedPomodoros, &app->unfinishedTimer,
         &app->unfinishedFullTimer);
...
void deleteLastLog(appData *app) {
  ...
  while (fgets(line, 1024, log) != NULL) {
    if (lineCounter != lastlineIndex - 1) fputs(line, tmp);
    lineCounter++;
  }
...
void writeToLog(appData *app) {
  int totalTime = ((app->pomodoros * app->workTime) +
                   ((app->pomodoros - 1) * app->shortPause) + app->longPause) /
                  (60 * 8);

  FILE *log;
  log = fopen(app->logFile, "a+");
  if (app->newDay == 1) fprintf(log, "%s\n", app->date);

  switch (app->currentMode) {
    case 0:
      if (app->cycles != 0 && app->needToLog == 1) {
        fprintf(log, "TT %dmin D %d %d %d\n", totalTime, app->workTime,
                app->shortPause, app->longPause);
      }
      break;

    case 1:
      fprintf(log, "%d/%d WT %d D %d %d %d\n", app->pomodoroCounter,
              app->pomodoros, (app->workTime - app->timer), app->workTime,
              app->shortPause, app->longPause);
      break;

    case 2:
      fprintf(log, "%d/%d SP %d D %d %d %d\n", app->pomodoroCounter,
              app->pomodoros, (app->shortPause - app->timer), app->workTime,
              app->shortPause, app->longPause);
      break;

    case 3:
      fprintf(log, "%d/%d LP %d D %d %d %d\n", app->pomodoros, app->pomodoros,
              (app->longPause - app->timer), app->workTime, app->shortPause,
              app->longPause);
      break;
    case -2:
      switch (app->lastMode) {
        ...
        case 3:
          fprintf(log, "%d/%d LP %d D %d %d %d\n", app->pomodoros,
                  app->pomodoros, (app->longPause - app->timer), app->workTime,
                  app->shortPause, app->longPause);
          break;
        default:
          break;
