...
void printCursor(appData* app) {
  ...
  char* cursorChar = "█";
  if (app->editingNote == 1 || app->editingTask == 1 || app->addingNote == 1 ||
      app->addingTask == 1)
    strcpy(cursorChar, "▏");
  mvprintw(app->middley - 9 + app->cursory, app->middlex - 13 + app->cursorx,
           "%s", cursorChar);
}
...
void printPartialNotes(appData* app, int max) {
  for (int i = max; i < app->notesAmount; i++) {
    ...
    col += 4;
    for (int j = 0; app->notes.lines[i]->note[j] != '\0'; j++) {
      if (j < app->cursorx || i != app->currentNote || app->addingNote == 1 ||
          app->addingTask == 1)
        mvprintw(row, col + j, "%c", app->notes.lines[i]->note[j]);
      else
        mvprintw(row, col + j + 1, "%c", app->notes.lines[i]->note[j]);
    }
  }
}
  ...
  if (app->addingNote == 1) {
    if (app->inputLength == 0)
      mvprintw((app->middley - 9) + app->notesAmount, (app->middlex - 17 + 4),
               "▏");
    mvprintw((app->middley - 9) + app->notesAmount, (app->middlex - 17 + 1),
             "%c", app->notes.lines[app->notesAmount]->type);
    for (int i = 0; i < app->inputLength; i++) {
      if (i < app->insertCursorx)
        mvprintw((app->middley - 9) + app->notesAmount,
                 (app->middlex - 17 + 4) + i, "%c",
                 app->notes.lines[app->notesAmount]->note[i]);
      else
        mvprintw((app->middley - 9) + app->notesAmount,
                 (app->middlex - 17 + 4) + i + 1, "%c",
                 app->notes.lines[app->notesAmount]->note[i]);
      mvprintw((app->middley - 9) + app->notesAmount,
               (app->middlex - 17 + 4) + app->insertCursorx, "▏");
    }
    ...
    if (app->inputLength == 0)
      mvprintw((app->middley - 9) + app->currentNote, (app->middlex - 17 + 4),
               "▏");
    mvprintw((app->middley - 9) + app->currentNote, (app->middlex - 17 + 1),
             "%c", app->notes.lines[app->currentNote]->type);
    for (int i = 0; i < app->inputLength; i++) {
      if (i < app->insertCursorx)
        mvprintw((app->middley - 9) + app->currentNote,
                 (app->middlex - 17 + 4) + i, "%c",
                 app->notes.lines[app->currentNote]->note[i]);
      else
        mvprintw((app->middley - 9) + app->currentNote,
                 (app->middlex - 17 + 4) + i + 1, "%c",
                 app->notes.lines[app->currentNote]->note[i]);
...
void printResume(appData* app) {
  if (app->needResume == 1) {
    printBanner(app);
    setColor(COLOR_WHITE, COLOR_BLACK, A_BOLD);
    mvprintw((app->middley - 2), (app->middlex - 16),
             "An unfinished cycle was detected!");
    if (strcmp(ICONS, "nerdicons") == 0) {
      mvprintw((app->middley - 1), (app->middlex - 16),
               " %d/%d🍅                    %02d/%02dm ",
    ...
    mvprintw(app->middley, (app->middlex - 16),
             "         Want to resume?         ");
    if (app->menuPos == 1) {
      setColor(COLOR_WHITE, COLOR_BLACK, A_BOLD);
...
void printPomodoroCounter(appData* app) {
  if (app->currentMode == 1)
    ...
    setColor(COLOR_CYAN, COLOR_BLACK, A_BOLD);
  mvprintw((app->middley - 7), (app->middlex + 10), "%02d",
           app->pomodoroCounter);
}
...
void printSettings(appData* app) {
  ...
  if (app->menuPos == 1) {
    setColor(COLOR_WHITE, COLOR_BLACK, A_BOLD);
    mvprintw((app->middley - 2), (app->middlex - 9), "<- pomodoros  %02d ->",
             app->pomodoros);
  } else {
    setColor(COLOR_WHITE, COLOR_BLACK, A_NORMAL);
    mvprintw((app->middley - 2), (app->middlex - 6), "pomodoros  %02d",
             app->pomodoros);
  }

  if (app->menuPos == 2) {
    setColor(COLOR_WHITE, COLOR_BLACK, A_BOLD);
    mvprintw((app->middley - 1), (app->middlex - 9), "<- work time %02dm ->",
             app->workTime / (60 * 8));
  } else {
    setColor(COLOR_WHITE, COLOR_BLACK, A_NORMAL);
    mvprintw((app->middley - 1), (app->middlex - 6), "work time %02dm",
             app->workTime / (60 * 8));
  }

  if (app->menuPos == 3) {
    setColor(COLOR_WHITE, COLOR_BLACK, A_BOLD);
    mvprintw((app->middley), (app->middlex - 10), "<- short pause %02dm ->",
             app->shortPause / (60 * 8));
  } else {
    setColor(COLOR_WHITE, COLOR_BLACK, A_NORMAL);
    mvprintw((app->middley), (app->middlex - 7), "short pause %02dm",
             app->shortPause / (60 * 8));
  }

  if (app->menuPos == 4) {
    setColor(COLOR_WHITE, COLOR_BLACK, A_BOLD);
    mvprintw((app->middley + 1), (app->middlex - 10), "<- long pause  %02dm ->",
             app->longPause / (60 * 8));
  } else {
    setColor(COLOR_WHITE, COLOR_BLACK, A_NORMAL);
    mvprintw((app->middley + 1), (app->middlex - 7), "long pause  %02dm",
             app->longPause / (60 * 8));
  }
  if (app->menuPos == 5) {
    setColor(COLOR_WHITE, COLOR_BLACK, A_BOLD);
    mvprintw((app->middley + 4), (app->middlex - 11),
             "-> back to main menu <-");
  } else {
    setColor(COLOR_WHITE, COLOR_BLACK, A_NORMAL);
...
void printTimer(appData* app) {
  ...
  if (app->currentMode == -2 && app->lastMode > 0)
    mode = app->lastMode;
  else if (app->currentMode == -2 && app->lastMode <= 0)
    return;
  FILE* time;
  if (TIMERLOG == 1) time = fopen(app->timerFile, "w");
